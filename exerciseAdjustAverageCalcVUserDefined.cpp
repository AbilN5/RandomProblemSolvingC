...
int main(){
	...
	conta_aprov = conta_rec = soma = conta_invalidos = media_geral = 0;
	...
	for(contador = 0; contador < alunos; contador++) {
		...
		//entrada/validação notas
		printf("Digite a nota 1: \n");
		if (scanf(" %f", &nota1) != 1) {
			scanf("%s", lixo);
			conta_invalidos++;
			printf("ERRO: Entrada inválida\nSeguindo para o próximo aluno...\n---------------------------\n");
		...
		//somar media ao total e determinar aprovação
		printf("Média: %.2f\n", media);
		soma+=media;
